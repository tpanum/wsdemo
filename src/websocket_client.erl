...
handle_info(timeout, #state{server={Host,Port,Path}} = State) ->
    ...
    case ssl:connect(Host,Port,
        ...
        {ok, Sock} ->
            ...
            {noreply, State#state{socket=Sock, readystate=?OPEN}};
        {error, timeout} ->
    ...
    case State#state.readystate of
        ?OPEN ->
            State2 = case unframe(<<SoFar/bits, Data/bits>>) of
                         {ok, Frame, SoFar2} ->
                             CBState = Mod:ws_onmessage(State, {binary, Data}, State#state.callback_state),
...
initial_request() ->
    ...
    io_lib:format("{\"command\":\"loogin\",\"params\":{\"username\":\"bjarke\",\"password\":\"12356\"},\"authd\":\"\",\"timestamp\":\"~p\"}\r\n", [MicroSecs]).
